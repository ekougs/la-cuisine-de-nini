...
def _compute_potential_dishes(dishes, rules, indices_to_compute, chosen_dishes):
    ...
    indices_to_compute = []
    for rule in rules:
        indices_to_compute = _check_rule(rule, indices_to_compute, chosen_dishes)
    return chosen_dishes, indices_to_compute
...
def get_dishes_of_day(get_dishes_fn=get_dishes, rules=_rules):
    ...
    while len(indices_to_compute) > 0:
        chosen_dishes, indices_to_compute = _compute_potential_dishes(dishes, rules, indices_to_compute, chosen_dishes)
